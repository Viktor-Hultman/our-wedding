let önskelista = [
    ...
    },
    {
        "namn": "PRESENTKORT",
        "pris": "",
        "inköpsställe": "IKEA, ELGIGANTEN, CERVERA",
        "länk": "",
        "harLänk": false,
        "tillgänglig": true
    }
...
function skapaLista(lista) {
    for(let i = 0; i < lista.length; i++) {
        if(lista[i].tillgänglig){
            ...
            let produktPris
            if(lista[i].pris != ""){
                produktPris = document.createElement("p");
                produktPris.classList.add("item-pris", "pris");
                produktPris.innerText = lista[i].pris + " kr"
            } else {
                produktPris = document.createElement("p");
                produktPris.classList.add("item-pris", "pris");
                produktPris.innerText = lista[i].pris
            }

            let produktInköpsställe
            if(!lista[i].harLänk){
                produktInköpsställe = document.createElement("p");
                produktInköpsställe.innerText = lista[i].inköpsställe
                produktInköpsställe.classList.add("item-inköpsställen", "inköpsställen");
            } else {
                produktInköpsställe = document.createElement("a");
                produktInköpsställe.innerText = lista[i].inköpsställe
                produktInköpsställe.classList.add("item-inköpsställen", "inköpsställen");
